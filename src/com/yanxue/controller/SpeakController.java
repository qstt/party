...
import com.yanxue.service.impl.SpeakServiceImpl;
...
public class SpeakController {
    @Resource
    SpeakServiceImpl service;

    @RequestMapping("/add.do")
    @ResponseBody
    public MyResult addStudent(Speak t,HttpServletRequest request) {
        String ipAddr = IpUtils.getIpAddr(request);
        t.setIp(ipAddr);
        return service.add(t);
    }
    
    @RequestMapping("/getSpeakData.do")
    @ResponseBody
    public MyResult getSpeakData(String lecid, Date time) {
        return service.getSpeakData(lecid, time);
    }
    
    @RequestMapping("/getSpeakData_1.do")
    @ResponseBody
    public MyResult getSpeakData_1(String lecid, Date time) {
        return service.getSpeakData_1(lecid, time);
    }
