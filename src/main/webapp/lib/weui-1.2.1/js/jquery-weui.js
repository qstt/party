...
+ function($) {
  ...
  //如果参数过多，建议通过 config 对象进行配置，而不是传入多个参数。
  $.pswPrompt = function(text, title, onOK, onCancel, input) {
        var config;
        if (typeof text === 'object') {
            config = text;
        } else {
        ...
        var modal = $.modal({
            text: '<p class="weui-prompt-text">'+(config.text || '')+'</p><input type="password" class="weui-input weui-prompt-input" id="weui-prompt-inputt" value="' + (config.input || '') + '" placeholder="输入密码"/>',
            title: config.title,
            autoClose: false,
            buttons: [
                {
                ...
                },
                {
                    ...
                    className: "primary",
                    onClick: function() {
                        var input = $("#weui-prompt-inputt").val();
                        if (!config.empty && (input === "" || input === null)) {
                            modal.find('.weui-prompt-input').focus()[0].select();
                            return false;
                        }
...
//如果参数过多，建议通过 config 对象进行配置，而不是传入多个参数。
    $.usernamePrompt = function(text, title, onOK, onCancel, input) {
        var config;
        if (typeof text === 'object') {
            config = text;
        } else {
        ...
        var modal = $.modal({
            text: '<p class="weui-prompt-text">'+(config.text || '')+'</p><input type="password" class="weui-input weui-prompt-input" id="weui-prompt-inputt" value="' + (config.input || '') + '" placeholder="输入用户名"/>',
            title: config.title,
            autoClose: false,
            buttons: [
                {
